...
const langs = ['nl', 'pl', 'ro', 'sv', 'da', 'no', 'fi', 'cs', 'hu', 'el', 'tr', 'ru', 'uk', 'ar', 'zh', 'ja', 'ko', 'ca', 'gl', 'eu'];
...
async function run() {
    ...
    for(const lang of langs) {
        ...
        // Translate in chunks of 5 keys to speed up
        for(let i = 0; i < keys.length; i += 5) {
            const chunkKeys = keys.slice(i, i+5);
            const promises = chunkKeys.map(k => translateText(es[k], lang));
            const results = await Promise.all(promises);
            for(let j=0; j<chunkKeys.length; j++){
                output += `    ${chunkKeys[j]}: ${JSON.stringify(results[j])},\n`;
            }
        }
        output += `  },`;
    }
